			...
			switch action {
				case .checkFaceId:
					if state.userSettings.faceIdEnabled {
						return .run { [localAuthenticationClient] send in
							try await send(.faceIdResponse(localAuthenticationClient.evaluate("JIJIJAJAJ")))
						}
					} else {
						return .none
					}
				...
				case .onAppear:
					return .merge(
						.send(.checkFaceId),
						.run { [localAuthenticationClient] send in
							try await send(.determine(localAuthenticationClient.determineType()))
						}
					)
				case .reset:
