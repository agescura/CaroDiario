		...
		Reduce { state, action in
			switch action {
				...
				case let .toggleFaceId(isOn: value):
					...
					return .run { [localAuthenticationType = state.userSettings.localAuthenticationType, localAuthenticationClient] send in
						try await send(.faceId(response: localAuthenticationClient.evaluate("Settings.Biometric.Test".localized(with: [localAuthenticationType.rawValue]))))
					}
